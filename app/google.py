...
from gdi_desktop_qgis.geopy.geocoders.base import Geocoder,GeocoderError,GeocoderResultError
from gdi_desktop_qgis.geopy import Point, Location, util
...
class Google(Geocoder):
    ...
    def url(self):
        ...
        return "https://{0}/maps/api/geocode/{{0}}?{{1}}".format(domain)
    ...
    def geocode(self, string, exactly_one=True):
        ...
        url = self.url.format(self.output_format.lower(), urlencode(params))
        url = self.sign(url)
        print(url)
    ...
    def parse_xml(self, page, exactly_one=True, reverse=False):
        ...
        def parse_place(place):
            ...
            addressDetails = place.getElementsByTagName('AddressDetails')
            accuracy = addressDetails[0].getAttribute("Accuracy")
    ...
    def parse_json(self, page, exactly_one=True, reverse=True):
        ...
        def parse_place(place):
            ...
            longitude, latitude = geo['location']['lat'], geo['location']['lng']
            print (latitude,longitude)
            return (location, (longitude, latitude), accuracy)
